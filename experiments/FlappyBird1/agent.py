        ...
        for episode in itertools.count():
            ...
            if is_training and episode % 100 == 0:
                ...
                if len(rewards_per_episode) > 0:
                    print(f"Average Reward: {np.mean(rewards_per_episode[-100:]):.2f}")
                print("-" * 30)

            # 1000 에피소드마다 또는 마지막 에피소드에서 모델 저장
            if is_training and episode > 0 and (episode % 1000 == 0 or (max_episodes and episode == max_episodes)):
                model_number = (episode + 999) // 1000  # 마지막 에피소드를 위해 수정
