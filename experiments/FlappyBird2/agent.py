...
class Agent():
    ...
    def run(self, is_training=True, render=False, max_episodes=None):
        ...
        num_states = env.observation_space.shape[0] # state의 개수
        num_actions = env.action_space.n # action의 개수
        
        rewards_per_episode = [] # 각 에피소드의 보상을 저장할 리스트
        epsilon_history = [] # epsilon 감소 기록을 저장할 리스트

        policy_dqn = DQN(num_states, num_actions, self.fc1_nodes).to(device) # 신경망 모델 초기화
